...
from colores import *
import datetime
from base_datos import guardar_estadisticas_preguntas_bdd, guardar_ranking_bdd, obtener_top10_bdd
from config import *
import config
...
def guardar_ranking(file_path, puntuacion, nombre_formateado, duracion_partida=None):
    """Guardar el ranking en SQLite(dejo file_path para no romper rutas y mantener compatibilidad)"""
    fecha_actual = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    try:
        guardar_ranking_bdd(nombre_formateado, int(puntuacion), fecha_actual, duracion_partida)
        print("✅Ranking guardado con exito!!")
    except Exception as e:
        print("Error guardando ranking en la BBDD", e)
...
def guardar_estadisticas_preguntas():
    """Antes lo guardaba en un archivo formato csv
    Ahora guarda y escribe las estadisticas actualizadas en BBDD
    """
    try:
        guardar_estadisticas_preguntas_bdd(estadisticas_preguntas)
    except Exception as e:
        print("✖️ Error al guardar estadistica", e)
...
def obtener_top_10(file_path):
    """Obtiene y retorna las 10 1ras filas con mayor puntaje de la tabla ranking de la BBDD"""
    try:
        filas = obtener_top10_bdd()
        return filas
    except Exception as e:
        print("✖️ Error al leer ranking en la BDD:", e)
        return []
